...
public class SpawnNetController : NetworkBehaviour
{
    ...
    private void CmdServerTransfer(int i, float x,float y,float z, int id)//нельзя передать значения извне, можно только работать с тем что уже есть на сервере
    ...
    {
        Vector3 installPosition = new Vector3(x, y, z);
    ...
    {
        GameController.OnServerSpawn(_numBuild, spawnPos.x,spawnPos.y,spawnPos.z, GameController.Player.PlayerNumber);
        print("--------------------------------------");
        /*Vector3 installPosition = new Vector3(spawnPos.x,spawnPos.y,spawnPos.z);
        GameObject inst = Instantiate(_prefabs[0], installPosition, rotation);
        inst.GetComponent<OwnerController>().SetOwner(0);
        NetworkServer.Spawn(inst);
        print("--------------------------------------");*/
