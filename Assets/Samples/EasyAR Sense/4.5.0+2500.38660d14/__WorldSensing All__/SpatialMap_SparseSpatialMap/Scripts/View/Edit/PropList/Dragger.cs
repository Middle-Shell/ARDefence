...
namespace SpatialMap_SparseSpatialMap
{
    public class Dragger : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (mapSession == null || mapSession.ARSession.Assembly == null || !mapSession.ARSession.Assembly.Camera) { return; }
            ...
            else
            {
                ...
                {
                    var ray = mapSession.ARSession.Assembly.Camera.ScreenPointToRay(Input.mousePosition);
                    RaycastHit hitInfo;
        ...
        public void SetFreeMove(bool free)
        {
            ...
            if (selection)
            {
                if (free)
                {
                    touchControl.TurnOn(selection.transform, mapSession.ARSession.Assembly.Camera, true, true, true, true);
                }
                else
                {
                    touchControl.TurnOn(selection.transform, mapSession.ARSession.Assembly.Camera, false, false, true, true);
                }
            }
        }
