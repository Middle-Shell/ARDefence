...
namespace easyar
{
    ...
    public class HuaweiAREngineFrameSource : FrameSource
    {
        ...
        void Update()
        {
            ...
            if (EnableColorInput)
            {
                pixelSize = ImageUtil.GetPixelSize(cameraImage.Y, cameraImage.U, cameraImage.V, cameraImage.YRowStride);
                pixelFormat = ImageUtil.CheckPixelFormat(cameraImage.Y, cameraImage.U, cameraImage.V, 1, cameraImage.UVPixelStride, cameraImage.UVPixelStride, cameraImage.YRowStride, cameraImage.UVRowStride, cameraImage.UVRowStride);
            }
            else
            {
                pixelSize = new Vector2Int(cameraImage.YRowStride, cameraImage.Height);
            ...
            ImageUtil.FillImageBuffer(Tuple.Create(cameraImage.Y, yLen), Tuple.Create(cameraImage.U, uvLen), Tuple.Create(cameraImage.V, uvLen), pixelSize, pixelFormat, buffer);
            cameraImage.Dispose();
