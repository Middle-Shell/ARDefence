...
namespace easyar
{
    ...
    public class NrealFrameSource : FrameSource
    {
        ...
        private void OnCameraFrameReceived(FrameRawData nrFrame)
        {
            ...
            var pixelSize = new Vector2Int(size.data_0, size.data_1);
            var pixelFormat = EnableColorInput ? PixelFormat.YUV_YV12 : PixelFormat.Gray;
            var yLen = pixelSize.x * pixelSize.y;
            var uvLen = yLen / 2;
            var bufferBlockSize = (pixelFormat == PixelFormat.Gray) ? yLen : uvLen * 3;

            if (yLen != nrFrame.data.Length * 2 / 3)
            {
                var message = $"Nreal YUV data assumption failed: ({nrFrame.data.Length} * 2 / 3 != {size.data_0} * {size.data_1}), EasyAR may not work properly";
