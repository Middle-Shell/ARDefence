...
namespace easyar
{
    ...
    public class MotionTrackerFrameSource : FrameSource
    {
        ...
        /// <summary>
        /// <para xml:lang="en">By default, <see cref="CheckAvailability"/> will return immediately if device is supported with current device list data, and the updated list will be used in the next run. Turn on this option If you want to ensure list update in each run.</para>
        /// <para xml:lang="zh">默认情况下，如果设备在当前列表数据下可以使用，<see cref="CheckAvailability"/>将立即返回，列表将在下次运行时更新。如果需要确保列表在每次运行时更新，可以打开这个选项。</para>
        /// </summary>
