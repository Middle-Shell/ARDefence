...
namespace easyar
{
    ...
    /// <para xml:lang="zh">在场景中控制ARCore相机设备（<see cref="ARCoreCameraDevice"/>）的<see cref="MonoBehaviour"/>，在Unity环境下提供功能扩展。如有需要可以直接使用<see cref="Device"/>。</para>
    ...
    public class ARCoreFrameSource : FrameSource
    {
        ...
        public override Optional<bool> IsAvailable
        {
            get
            {
                if (Application.platform == RuntimePlatform.Android && EasyARSettings.Instance && EasyARSettings.Instance.DisableARCore)
                {
                    GUIPopup.EnqueueMessage("ARCore is manually disabled. See EasyAR > Sense > Configuration > Disable ARCore.", 3);
                    return false;
                }
