...
namespace easyar
{
    ...
    public class CloudSpatialMapLocalizerFrameFilter : FrameFilter
    {
        ...
        /// </summary>
        public int ResolveTimeout = 6000;
            ...
            session.FrameUpdate += (outputFrame) =>
            {
                ...
                while (pendingRequets.Count > 0)
                {
                    using (var iFrame = outputFrame.inputFrame())
                    {
                        ...
                        if (iFrame.hasSpatialInformation() && iFrame.trackingStatus() != MotionTrackingStatus.Tracking)
                        {
                            request.FinishCallback?.Invoke(null, $"Skip underlying resolve: tracking status = {iFrame.trackingStatus()}", 0, Optional<Matrix44F>.Empty);
                            continue;
                        }
                        ...
                        cloudLocalizer.resolve(iFrame, request.Message, acceleration, location, ResolveTimeout, EasyARController.Scheduler, (result) => { request.FinishCallback(result, string.Empty, timestamp, cameraToVIOOrigin); });
                    }
                }
            };
        ...
        public void ResolveRaw(Optional<Input> input, Action<InputFrame> start, Action<Optional<CloudLocalizeResult>, string> finish)
        {
            ...
            if (cloudLocalizer == null)
            {
                if (failToCreate)
                {
                    finish?.Invoke(null, "Skip underlying resolve: localizer fail to create");
                }
                else
                {
            ...
            var request = new Request
            {
                ...
                FinishCallback = (resultO, error, timestamp, cameraToVIOOrigin) =>
                {
                    ...
                    if (result.getLocalizeStatus() == CloudLocalizeStatus.FoundMaps)
                    {
                        ...
                        {
                            // block id is obsolete, only for v3
                            ID = string.IsNullOrEmpty(result.getLocalizedBlockId()) ? result.getLocalizedMapID() : result.getLocalizedBlockId(),
        ...
        private IEnumerator AutoCreate()
        {
            ...
            if (UseGlobalServiceConfig)
            {
                ...
                {
                    config = EasyARSettings.Instance.GlobalCloudLocalizerServeiceConfig;
                }
            }
            else
            {
