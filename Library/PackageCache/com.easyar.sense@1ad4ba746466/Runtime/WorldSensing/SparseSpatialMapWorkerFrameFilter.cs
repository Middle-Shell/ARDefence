...
namespace easyar
{
    ...
    public class SparseSpatialMapWorkerFrameFilter : FrameFilter, FrameFilter.IInputFrameSink, FrameFilter.IOutputFrameSource
    {
        ...
        /// <summary>
        /// <para xml:lang="en">Timeout in milliseconds when loading map from server.</para>
        /// <para xml:lang="zh">从服务器加载地图的超时时间（毫秒）。</para>
        /// </summary>
        public int LoadTimeout = 60000;

        /// <summary>
        /// <para xml:lang="en">Timeout in milliseconds when uploading map to server.</para>
        /// <para xml:lang="zh">往服务器上传地图的超时时间（毫秒）。</para>
        /// </summary>
        ...
        public override void UpdateMotion(double timestamp, MotionTrackingStatus trackingStatus, Matrix44F cameraTransform)
        {
            ...
            if (WorkingMode == Mode.Localize && EnablePoseStabilizer && localizedMapPose.OnSome)
            {
                ...
                {
                    if (trackingStatus == MotionTrackingStatus.Tracking)
                    {
        ...
        internal void LoadSparseSpatialMap(SparseSpatialMapController controller, Action<SparseSpatialMapController.SparseSpatialMapInfo, bool, string> callback)
        {
            ...
            if (UseGlobalServiceConfig)
            {
                ...
                {
                    config = EasyARSettings.Instance.GlobalSpatialMapServiceConfig;
                }
            }
            else
            {
            ...
            Manager.load(sparseSpatialMapWorker, id.Trim(), config.APIKey.Trim(), config.APISecret.Trim(), config.SparseSpatialMapAppID.Trim(), LoadTimeout, EasyARController.Scheduler, (status, error) =>
            {
                if (MapLoad != null)
                {
        ...
        internal void HostSparseSpatialMap(SparseSpatialMapController controller, string name, Optional<Image> preview, Action<SparseSpatialMapController.SparseSpatialMapInfo, bool, string> callback)
        {
            ...
            Manager.host(sparseSpatialMapWorker, config.APIKey.Trim(), config.APISecret.Trim(), config.SparseSpatialMapAppID.Trim(), name, preview, HostTimeout, EasyARController.Scheduler, (status, id, error) =>
            {
        ...
        public class MapLocalizerConfig
        {
            ...
            /// <para xml:lang="zh">定位模式。如果<see cref="EnablePoseStabilizer"/>是true，会强制为<see cref="LocalizationMode.KeepUpdate"/>。</para>
            /// </summary>
